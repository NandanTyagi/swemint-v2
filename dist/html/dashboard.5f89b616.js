...
function fetchNftMetadata(nfts) {
    ...
    for(let i = 0; i < nfts.length; i++){
        ...
        promises.push(fetch(`${serverUrl}/functions/getNFT?_ApplicationId=${appId}&nftId=${nftId}`).then((res)=>res.json()).then((res)=>JSON.parse(res.result)).then((res)=>nft.metadata = res).then(()=>{
            const options = {
                address: contractAddress,
                token_id: nftId,
                chain: chain
...
//# sourceMappingURL=dashboard.5f89b616.js.map
